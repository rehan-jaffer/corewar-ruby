...
def null_program(size)
  0.upto(size - 1).map do |i|
    Core::Instructions::DummyInstruction.new(tag: "Instruction #{i}")
  end
end
...
describe Core::VM do
  describe "initialization" do
    ...
    it "shouldn't boot if there's no program loaded" do
      core = Core::VM.new(core_size: 16)
      expect { core.boot! }.to raise_error Core::EmptyCoreException
    end
  end
...
#    end
  ...
  describe "process queue" do
    ...
    it "creates a process queue with a process counter when a program is loaded" do
      core = Core::VM.new(core_size: 1024)
      program = Core::Program.new(null_program(16), program_id: 1)
      core.load(program)
    ...
    it "executes a cycle, updating the process count in the queue" do
      core = Core::VM.new(core_size: 1024)
      core.load(Core::Program.new(null_program(16), program_id: 1))
      core.boot!
    ...
    it "creates a process queue entry for each new process" do
